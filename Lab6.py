...
def matrixGenerator():
    def f(X1, X2, X3):
        y = 3.1 + 6.3 * X1 + 9.8 * X2 + 5.5 * X3 + 2.5 * X1 * X1 + 0.4 * X2 * X2 + 1.0 * X3 * X3 + 3.5 * X1 * X2 + 0.7 * X1 * X3 + 7.9 * X2 * X3 + 8.7 * X1 * X2 * X3 + randrange(0, 10) - 5
        return y
...
def critCohrenValue(selectionSize, qty_of_selections, significance):
    selectionSize += 1
    ...
    for column in range(number_x + 1):
        ...
        for row in range(N):
            if column == 0:
                ...
                t_practice += middle_y[row] * matrix_of_planning[row][column - 1]
...
def fisher_test():
    ...
    F_theoretical = critFisherValue(f3, f4, q)
...
for i in range(len(matrix_x)):
    if i < 8:
        x_1 = min_x1 if matrix_of_planning[i][0] == -1 else max_x1
        x_2 = min_x2 if matrix_of_planning[i][1] == -1 else max_x2
        x_3 = min_x3 if matrix_of_planning[i][2] == -1 else max_x3
    else:
        arr_x = x(matrix_of_planning[i][0], matrix_of_planning[i][1], matrix_of_planning[i][2])
        x_1, x_2, x_3 = arr_x
...
while numberOfCycles:
    ...
    mx_i = middle_x  # Список середніх значень колонок
    my = sum(middle_y) / 15
    values = [
    ...
    print("{:.3f} + {:.3f} * X1 + {:.3f} * X2 + {:.3f} * X3 + {:.3f} * Х1X2 + {:.3f} * Х1X3 + {:.3f} * Х2X3"
          "+ {:.3f} * Х1Х2X3 + {:.3f} * X11^2 + {:.3f} * X22^2 + {:.3f} * X33^2 = ^y"
          .format(beta[0], beta[1], beta[2], beta[3], beta[4], beta[5], beta[6], beta[7], beta[8], beta[9], beta[10]))
    for i in range(N):
        print("^y{} = {:.3f} ~= {:.3f}".format((i + 1), check_result(beta, i), middle_y[i]))
    ...
    print("Перевірка")
    for i in range(N):
        print("^y{} = {:.3f} ~= {:.3f}".format((i + 1), check_result(student_arr, i), middle_y[i]))
